...
class Graph:
    ...
    def add_vertex(self, node):
        ...
        self.vert_list.append(int(node.get_id()))
        return node
...
class MapNavigator(object):
    ...
    def startup(self, startBlock, startLevel, startId, endBlock, endLevel, endId):
        ...
        #    endLevel = raw_input()
            ...
            for eachMap in buildingInfo:
                counter +=1
                if int(eachMap[0]) == endBlock and int(eachMap[1]) == endLevel:
                    counter -=1
    ...
    def shortestPathInSingleMap(self,graph,startNode,endNode):
        self.dijkstra(graph, startNode, endNode)
        target =  endNode
        path = [target.get_id()]
    ...
    def nextMapInfo(self,graph,mapGraph,thisNode):
         nodeList = graph.get_list()
         found = False
         for eachNode in nodeList:
            name = graph.get_vertex(eachNode).get_name().lower()
            
            if "-" in name and "to " in name and graph.get_vertex(int(eachNode)).get_id() != thisNode:
                print "in"
                ...
                for eachGraph in mapGraph:
                    ...
                    if int(eachGraph[0].get_block()) == int(name[0]) and int(eachGraph[0].get_level()) == int(name[1]):
                        midpointEndNode = eachNode
                        print "inside found"
                        found = True
                        break
                        #block,level, midpoint end node on this map, midpoint start node on next map
            if found:
                break
                
         return [name[0], name[1], midpointEndNode, name[2]]

    def shortestPath(self, mapGraph,startNode,endNode):
        pathArray = []
        mapArray = []
        nextBlock = -sys.maxint
        nextLevel = -sys.maxint
        midpointEndNode= sys.maxint
        midpointStartNode= sys.maxint
        canBreak = False

        for eachGraph in mapGraph:
            if eachGraph[-1]== "Same":
                path = self.shortestPathinSingleMap(eachGraph[0],eachGraph[0].get_vertex(startNode), eachGraph[0].get_vertex(endNode))
                pathArray.append(path)
                mapArray.append (eachGraph[0])
                break
            if eachGraph[-1] == "Start":
                nextMap = self.nextMapInfo(eachGraph[0],mapGraph,startNode)
                nextBlock = int(nextMap[0])
                nextLevel = int(nextMap[1])
                midpointEndNode = int(nextMap[2])
                midpointStartNode = int(nextMap[3])

                path = self.shortestPathInSingleMap(eachGraph[0],eachGraph[0].get_vertex(startNode), eachGraph[0].get_vertex(midpointEndNode))
                pathArray.append(path)
                mapArray.append (eachGraph[0])  
                i = 0

                while i < len(mapGraph):

                    if int(mapGraph[i][0].get_block()) == nextBlock and int(mapGraph[i][0].get_level()) == nextLevel:

                        if mapGraph[i][-1] =="End":
                            path = self.shortestPathInSingleMap(mapGraph[i][0],mapGraph[i][0].get_vertex(midpointStartNode), mapGraph[i][0].get_vertex(endNode))
                            pathArray.append(path)
                            mapArray.append(mapGraph[i][0])
                            canBreak = True
                            break
                        else:

                            nextMap = self.nextMapInfo(mapGraph[i][0],mapGraph,midpointStartNode)
                            midpointEndNode = int(nextMap[2])
                            path = self.shortestPathInSingleMap(mapGraph[i][0],mapGraph[i][0].get_vertex(midpointStartNode), mapGraph[i][0].get_vertex(midpointEndNode))
                            nextBlock = int(nextMap[0])
                            nextLevel = int(nextMap[1])
                            midpointStartNode = int(nextMap[3])          
                            pathArray.append(path)
                    ...
                    i+=1
        return [pathArray,mapArray]
                    
    def travelDirection(self, currentDirection, currentX, currentY, pathArray, mapArray):
        nextX =mapArray[0].get_vertex(int(pathArray[0][0])).get_x()
        nextY = mapArray[0].get_vertex(int(pathArray[0][0])).get_y()
         
        distance = math.sqrt(math.pow((currentX-nextX),2)+ math.pow((currentY-nextY),2))
        if distance < 60:
            # reset x and y
            currentNode = pathArray[0].pop(0)
            print 'You have reached node' ,currentNode
            AudioManager.play('node')
            AudioManager.playNumber(currentNode)
            if len(pathArray[0])==0 and len(pathArray)==1:
        ...
        NorthToVert = 360 - mapArray[0].get_north()
...
info = MapNavigator.startup(1,2,1,2,3,15)

mapGraph =MapNavigator.parseInfo(info[2],info[3])
print "here 1"
output = MapNavigator.shortestPath(mapGraph, info[0],info[1])
print "here"
print output